...
# Msg(Mulit-scale grouping)
class PointNetSetAbstractionMsg(nn.Module):
    def __init__(self, npoint, radius_list, nsample_list, in_channel, mlp_list, knn=False):
        super(PointNetSetAbstractionMsg, self).__init__()
        self.npoint = npoint  # FPS中最终需要点的数量
        self.radius_list = radius_list  # 球查询的半径,一个点要采用不同半径进行多尺度采样MSG
        self.nsample_list = nsample_list  # 围绕每个采样的点为中心以指定的半径进行采样，每个区域中点数量上限
        self.conv_blocks = nn.ModuleList()
        self.bn_blocks = nn.ModuleList()
        self.out_channel = 0  # 每个点MLP输出的尺度
    ...
    def forward(self, xyz, points):
        ...
        B, C, N = xyz.shape
        S = self.npoint
        fps_idx = farthest_point_sample(xyz.permute(0, 2, 1), S)  # permute交换对应维度，维度1和2交换，原本的xyz[i][m][n]变成xyz[i][n][m]
